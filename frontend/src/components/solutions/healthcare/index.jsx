import { healthcareSolutionsData as data } from "../../../data/solution_healthcare_data/data";
import {
  Activity,
  XSquare,
  Layout,
...
const solutionIcons = [Zap, Users, Layout, Palette, RefreshCcw];

// Benefits icons
const benefitIcons = [TrendingUp, Zap, Layout, RefreshCcw];

// Use case icons
const useCaseIcons = [Activity, Heart, ShieldPlus, Users];
    ...
    <div className="w-full bg-light-bg dark:bg-dark-bg font-dm flex flex-col pt-16 pb-24 text-light-text1 dark:text-dark-text1 overflow-x-hidden">
      ...
      <section className="px-6 max-w-7xl mx-auto w-full flex flex-col lg:flex-row items-center gap-16 lg:gap-8 mb-20 ">
        ...
        <div className="flex-1 text-center lg:text-left flex flex-col items-center lg:items-start">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-cyan-500/30 bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 text-xs font-semibold uppercase tracking-widest mb-6">
            <Activity size={14} />
            {data.hero.badge}
          </span>
          ...
          <p className="text-light-text2 dark:text-dark-text2 text-lg mt-6 max-w-lg leading-relaxed">
            Deploy professional, highly-secure identity badges for fast-moving
            hospital staff, interns, and patients with rigorous tracking and
            rapid updates.
          </p>
          <div className="mt-8 flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            <button className="px-8 py-3.5 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl font-bold shadow-lg shadow-cyan-500/30 transition-transform hover:-translate-y-1">
              Start for Free
            </button>
        ...
        <div className="flex-1 relative flex items-center justify-center min-h-[460px] w-full mt-8 lg:mt-0 width_425px_hidden animate-floatCard drop-shadow-2xl">
          <div className="absolute w-[350px] h-[350px] rounded-full bg-cyan-500/10 blur-[100px] animate-pulseSlow" />
          ...
          <div className="relative z-10 w-[300px] h-[220px] rounded-[2rem] bg-light-card1/90 dark:bg-dark-card1/90 backdrop-blur-xl border border-light-border dark:border-dark-border shadow-2xl flex flex-col items-center justify-center gap-4 group">
            <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/[0.02] to-transparent pointer-events-none rounded-[2rem] group-hover:bg-cyan-500/[0.05] transition-colors duration-500" />
            <div className="flex items-center gap-3 mb-2 opacity-80">
              <Activity size={24} className="text-cyan-500" />
              <span className="font-bold text-lg tracking-widest text-light-text1 dark:text-white uppercase transition-all duration-300 group-hover:tracking-[0.2em]">
                Hospital Node
              </span>
            </div>
          ...
          {/* Animated Floating ID 1 - Staff Info */}
          <div
            className="absolute z-20 top-4 right-4 lg:right-12 p-3 rounded-2xl bg-white dark:bg-[#1a1a1e] border border-light-border dark:border-white/10 shadow-xl animate-floatCard w-[140px] rotate-12"
          ...
          >
            <div className="w-full aspect-[3/4] bg-gradient-to-b from-blue-500/20 to-transparent rounded-xl flex flex-col items-center p-3 relative overflow-hidden text-blue-500 border border-blue-500/20">
              <Activity size={20} className="mt-2" />
              <div className="w-full h-1.5 bg-blue-500/30 rounded-full mt-4" />
              <div className="w-2/3 h-1 bg-blue-500/20 rounded-full mt-2" />
              <span className="absolute bottom-5 text-[8px] font-black tracking-widest uppercase">
                Physician
              </span>
            </div>
          </div>

          {/* Animated Floating ID 2 - Patient */}
          <div
            className="absolute z-30 bottom-12 left-4 lg:left-8 p-3 rounded-2xl bg-white dark:bg-[#1a1a1e] border border-cyan-500/40 shadow-[0_10px_40px_-5px_rgba(6,182,212,0.3)] animate-floatCard w-[140px] -rotate-6"
            style={{ animationDelay: "1000ms" }}
          >
            <div className="w-full aspect-[3/4] bg-gradient-to-b from-cyan-600 to-teal-600 rounded-xl flex flex-col items-center p-3 text-white">
              ...
              <span className="absolute bottom-5 text-[8px] font-black tracking-widest uppercase text-white/90">
                Patient
              </span>
            </div>
          </div>
          ...
          <div className="absolute z-0 flex items-center justify-center opacity-30 pointer-events-none">
            <div className="w-[300px] h-[300px] border-2 border-dashed border-cyan-500/20 rounded-full animate-[spin_30s_linear_infinite]" />
      ...
      <section className="px-6 max-w-7xl mx-auto w-full flex flex-col lg:flex-row gap-8 mb-24 animate-slideUp">
        ...
        <div className="flex-[3] p-10 rounded-[2.5rem] bg-rose-500/5 border border-rose-500/10 shadow-lg relative overflow-hidden group">
          ...
          <div className="space-y-4 relative z-10">
            {data.challenge.items.map((item, idx) => (
              <div
                key={idx}
                className="flex gap-4 items-center p-4 rounded-2xl bg-white/50 dark:bg-black/20 backdrop-blur-md border border-light-border dark:border-white/5 transition-transform duration-300 hover:-translate-y-1"
              >
                <div className="w-8 h-8 rounded-full bg-rose-500/10 flex items-center justify-center text-rose-500 shrink-0 shadow-inner">
                  <XSquare size={14} />
                </div>
                <span className="font-medium text-[0.95rem] text-light-text1 dark:text-gray-300 leading-relaxed">
                  {item}
                </span>
              </div>
            ))}
        ...
        <div className="flex-[2] flex flex-col gap-8">
          <div className="p-10 rounded-[2.5rem] bg-light-card1 dark:bg-dark-card1 border border-light-border dark:border-dark-border shadow-lg h-full flex flex-col justify-center relative overflow-hidden">
            <div className="absolute top-0 right-0 w-[200px] h-[200px] bg-cyan-500/10 rounded-full blur-[60px] pointer-events-none" />
            ...
            <div className="flex flex-col gap-4 relative z-10">
              {data.useCases.items.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex items-center gap-4 bg-light-bg dark:bg-[#151515] p-5 rounded-2xl border border-light-border dark:border-white/5 shadow-sm transition-all hover:border-cyan-500/30 cursor-default hover:scale-[1.02]"
                  >
      ...
      >
        <div className="mb-12 text-center md:text-left flex items-center justify-center md:justify-start gap-3">
          <div className="w-10 h-10 rounded-full bg-cyan-500/10 flex items-center justify-center hidden md:flex">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
          {data.solution.features.map((feature, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`p-8 rounded-[2.5rem] bg-gradient-to-br from-light-card1 to-light-bg dark:from-dark-card1 dark:to-[#0f0f12] border border-light-border dark:border-dark-border shadow-md hover:shadow-2xl hover:shadow-cyan-500/10 hover:-translate-y-1 transition-all duration-300 flex flex-col gap-4 group ${spanClass}`}
              >
                <div className="w-14 h-14 rounded-[1rem] bg-cyan-500/10 border border-cyan-500/10 flex items-center justify-center text-cyan-500 transition-all duration-300 group-hover:bg-cyan-500 group-hover:text-white group-hover:scale-110 shadow-inner">
                  <Icon size={24} />
                </div>
      ...
      <section
        className="px-6 max-w-7xl mx-auto w-full animate-slideUp"
      ...
      >
        <div className="p-10 xl:p-16 rounded-[3rem] bg-[#0A0A0B] border border-white/10 shadow-2xl relative overflow-hidden">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-[radial-gradient(ellipse_at_center,rgba(6,182,212,0.15)_0%,transparent_70%)] pointer-events-none" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 relative z-10">
            {data.benefits.items.map((benefit, idx) => {
              ...
              return (
                ...
                >
                  <div className="w-[72px] h-[72px] rounded-[1.5rem] bg-white/5 border border-white/10 flex items-center justify-center text-cyan-400 backdrop-blur-xl group-hover:scale-110 transition-transform duration-300 shadow-[0_0_30px_-5px_rgba(6,182,212,0.2)]">
                    <Icon size={28} />
                  </div>
                  <h4 className="text-white font-bold text-[1.1rem] px-2">
                    {benefit.name}
                  </h4>
