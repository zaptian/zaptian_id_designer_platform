...
import { cookiesData as data } from "../../../data/legal_cookies_data/data";
        ...
        <div className="hidden lg:block w-72 shrink-0">
          <div className="sticky top-28 flex flex-col gap-2 p-6 rounded-3xl bg-white dark:bg-[#0c0c0e]/80 backdrop-blur-md border border-light-border dark:border-white/5 shadow-xl">
            <h3 className="font-bold text-sm tracking-widest uppercase mb-4 text-light-text1 dark:text-white flex items-center gap-2">
               <Cookie size={16} className="text-button-primary" />
               Contents
            </h3>
            ...
            {data.sections.map((section) => (
              <section key={section.id} id={section.id} className="scroll-mt-32">
                ...
                <div className="flex flex-col gap-5 text-base md:text-[17px] leading-relaxed text-light-text2 dark:text-gray-300 font-medium">
                  ...
                  {/* Structural Table Blocks */}
                  {section.table && section.table.length > 0 && (
                    <div className="overflow-x-auto mt-2">
                      <table className="w-full text-left border-collapse rounded-xl overflow-hidden shadow-sm border border-light-border dark:border-white/5">
                        <thead className="bg-[#f0f4f8] dark:bg-[#111216]">
                          <tr>
                            <th className="px-6 py-4 w-1/3 text-xs md:text-sm font-black text-light-text1 dark:text-white uppercase tracking-widest block sm:table-cell">Type</th>
                            <th className="px-6 py-4 text-xs md:text-sm font-black text-light-text1 dark:text-white uppercase tracking-widest block sm:table-cell">Purpose</th>
                          </tr>
                        </thead>
                        <tbody className="bg-white dark:bg-[#0c0c0e]/80 divide-y divide-light-border dark:divide-white/5">
                          {section.table.map((row, i) => (
                            <tr key={i} className="hover:bg-light-hover dark:hover:bg-white/[0.02] transition-colors block sm:table-row">
                              <td className="px-6 py-4 font-bold text-light-text1 dark:text-white block sm:table-cell border-b sm:border-0 border-light-border/30 dark:border-white/5">
                                 {row.type}
                              </td>
                              <td className="px-6 py-4 leading-snug opacity-90 block sm:table-cell text-[15px] md:text-base">
                                 {row.purpose}
                              </td>
                            </tr>
                          ))}
